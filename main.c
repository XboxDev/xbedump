...
/* title */
    printf("XBE dumper 1.0 (C)2002 Michael Steil mist@c64.org\n\n");

/* parse command line */
    filename = "xboxdash.xbe";
    warn = 1;
...
//    if (warn) if (header->ImageSize != filesize) printf("** should be equal to file size! %x\n", filesize);
    ...
    printf("Alternate signature keys: "); printhexm((char*)cert->AlternateSignatureKeys, sizeof(cert->AlternateSignatureKeys));

    sechdr = xbe + (int)header->Sections - (int)header->BaseAddress;
    for (i = 0; i < header->NumSections; i++, sechdr++) {
        ...
        printf("File address:\n\t0x%x\n", sechdr->FileAddress);
        printf("File size:\n\t0x%x\n", sechdr->FileSize);
        printf("Section name:\n\t%s\n", xbe+(int)sechdr->SectionName-(int)header->BaseAddress);
        printf("Section reference count:\n\t%i\n", sechdr->SectionReferenceCount);
        printf("Pointer to head shared page reference count:\n\t0x%x\n", sechdr->HeadReferenceCount);
        printf("Pointer to tail shared page reference count:\n\t0x%x\n", sechdr->TailReferenceCount);
        printf("SHA hash: "); printhex32m(sechdr->ShaHash, 5);
    }

    tls = xbe + (int)header->TlsDirectory - (int)header->BaseAddress;
...
//    printf("Characteristics:\n\t\"%s\"\n",xbe+(int)tls->Characteristics-(int)header->BaseAddress);
    ...
    lib = xbe + (int)header->Libraries - (int)header->BaseAddress;
